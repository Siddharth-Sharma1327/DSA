...
{   private:
	...
	vector<int> topoSort(int V, vector<int> adj[]) 
	{
	   ...
	   while(!q.empty()){
	       ...
	       q.pop();
	       topo.push_back(node);       //after poping node form the queue it is pushed into the answer vector
...
// Sapce Complexity--> O(V) + O(V +O(V (for indegree array, queue, vector)
// Time Complexity--> O(V) + O(V+E) (for the queue and for the first for loop as it is directed graph edges will be E(=indegree))